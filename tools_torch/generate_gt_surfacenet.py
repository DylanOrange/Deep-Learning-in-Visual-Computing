...
def parse_args():
    ...
    parser.add_argument("--data_path", metavar="DIR", help='path to raw data', default='/content/drive/MyDrive/Deep-Learning-in-Visual-Computing/scannet')
...
def save_tsdf(args, scene_path, cam_intr, depth_list, cam_pose_list, color_list, name,save_mesh=False):
    ...
    tsdf_vol = TSDFVolumeTorch(vol_bnds, voxel_size=args.voxel_size, margin=args.margin)
    ...
    tsdf_info = {
        'vol_origin': tsdf_vol._vol_origin.cpu().numpy(),
        'voxel_size': tsdf_vol._voxel_size.cpu().numpy(),
        'vol_bounds': tsdf_vol._vol_bonds.cpu().numpy(),
    }
    tsdf_path = os.path.join(args.save_path, scene_path)
    if not os.path.exists(tsdf_path):
        os.makedirs(tsdf_path)

    with open(os.path.join(args.save_path, scene_path, 'tsdf_info_'+str(name)+'.pkl'), 'wb') as f:
        pickle.dump(tsdf_info, f)
    tsdf_volume, weight_volume, occ_volume,_= tsdf_vol.get_volume()
    tsdf_volume = tsdf_volume.cpu().numpy()
    weight_volume = weight_volume.cpu().numpy()
    occ_volume  = occ_volume.cpu().numpy()
    writeocc(occ_volume,os.path.join(args.save_path, scene_path, 'occ_mesh'+str(name)+'.ply'))
    np.savez_compressed(os.path.join(args.save_path, scene_path, 'tsdf_'+str(name)), tsdf_volume)
    np.savez_compressed(os.path.join(args.save_path, scene_path, 'occ_'+str(name)), occ_volume)

    if save_mesh:
        print("Saving mesh to mesh.ply...")
        verts, faces, norms, colors = tsdf_vol.get_mesh()

        meshwrite(os.path.join(args.save_path, scene_path, 'mesh'+str(name)+'.ply'), verts, faces, norms,
                  colors)


def save_fragment_pkl(args, scene, cam_intr, depth_list,
                      cam_pose_list,name):  # scene:str  cam_intr:matrix    depth_list:dict   cam_pose_list:
    fragments = []
    ...
    for id in depth_list.keys():
        ...
        with open(os.path.join(args.save_path, scene, 'tsdf_info_'+str(name)+'.pkl'), 'rb') as f:
        ...
        with open(os.path.join(args.save_path, scene, 'fragments_'+str(name)+'.pkl'), 'wb') as f:
...
def process(args, scannet_files):  # scannet_files contains different scene
    for scene in tqdm(scannet_files):
        ...
        for id, (cam_pose, depth_im, color_image) in enumerate(dataloader):
            ...
            count+=1
            if count % 10 == 0:
                save_tsdf(args,scene,cam_intr,depth_all,cam_pose_all,color_all,count,save_mesh=True)
                save_fragment_pkl(args,scene,cam_intr,depth_all,cam_pose_all,count)
                print("generate tsdf and occ {}/{}".format(str(id),str(n_imgs)))
                depth_all = {}
                cam_pose_all = {}
                color_all = {}
